...
const onModalBtnClickHandler = e => {
  ...
  if (
    currentTarget.classList.contains('info') &&
    target.classList.contains('ok-btn')
  ) {
    removeModal();
    console.log('You were successfully subscribed');
    return;
  }
  if (
    currentTarget.classList.contains('alert') &&
    target.classList.contains('ok-btn')
  ) {
    removeModal();
...
const a = document.querySelector('.modal__overlay');
a.addEventListener('click', onModalBtnClickHandler);
